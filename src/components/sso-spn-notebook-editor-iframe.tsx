...
/**
 * SSO-SPN specific server-side component that fetches the user's Databricks SPN token
 * and generates an encrypted proxy URL for the notebook editor iframe.
 *
 * This version redirects to /sso-spn on error instead of /databricks-idp
 */
export default async function SsoSpnNotebookEditorIframe() {
  // Fetch the user's Databricks workspace token from the session
  // getDatabricksWorkspaceToken automatically detects SPN users and uses SPN credentials
  const tokenResult = await getDatabricksWorkspaceToken();

  if (!tokenResult.success) {
    // Redirect to SPN login if no token is available
  ...
  if (!appUrl) {
    throw new Error("DATABRICKS_NOTEBOOK_APP_URL environment variable is required");
  }
