...
def backend_command(backend_args: list[str] | None = None) -> list[str]:
    """Console script on Apps (pip install); uv locally."""
    if shutil.which("start-server"):
        cmd = ["start-server"]
    else:
...
class ProcessManager:
    ...
    def apply_chat_ui_patches(self):
        """Overlay Firefly-specific chat UI fixes onto the cloned template."""
        patch_root = Path(__file__).resolve().parent.parent / "patches" / "e2e-chatbot-app-next"
        target_root = Path("e2e-chatbot-app-next")
        if not patch_root.is_dir() or not target_root.is_dir():
            return
        ...
        print("Applied chat UI patches from patches/e2e-chatbot-app-next")

    def clone_frontend_if_needed(self):
        if Path("e2e-chatbot-app-next").exists():
            self.apply_chat_ui_patches()
            return True
