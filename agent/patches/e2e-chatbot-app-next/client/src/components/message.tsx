...
import { cn, sanitizeText, formatToolOutput } from '@/lib/utils';
...
import { GenieAttribution } from './genie-attribution';
import {
  collectGenieLinksFromMessage,
  extractGenieLinks,
  isGenieToolPart,
} from '@/lib/genie-attribution';
...
const PurePreviewMessage = ({
  ...
  const { genieConfig } = useAppConfig();
  const genieLinks = React.useMemo(
    () => collectGenieLinksFromMessage(message.parts),
    [message.parts],
  );
  const showGenieAttribution =
    message.role === 'assistant' && !!genieConfig;
...
const ToolPartRenderer = ({
  ...
  const isGenieTool = isGenieToolPart(part);
  const genieLinks =
    isGenieTool && output != null ? extractGenieLinks(output) : [];
  ...
  if (isMcpApproval) {
    return (
      <McpTool defaultOpen={state !== 'output-available'}>
        <McpToolHeader
